...
class ReturnController {
  async list(req, res) {
    try {
      ...
      const result = await returnService.getList(queryParams);
      res.status(200).json({
        total: result.count,
  ...
  async create(req, res) {
    try {
      const record = await returnService.create(req.body);
      res.status(201).json(record);
    } catch (error) {
      res
        .status(500)
        .json({ message: "Error creating return", error: error.message });
    }
  }
  ...
  async getById(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error fetching return", error: error.message });
    }
  }

  async update(req, res) {
    try {
      const updated = await returnService.alter(req.params.id, req.body);
      if (!updated)
      ...
      res
        .status(500)
        .json({ message: "Error updating return", error: error.message });
    }
  }
  ...
  async delete(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error deleting return", error: error.message });
    }
  }

  async complete(req, res) {
    try {
      const updated = await returnService.completeReturn(req.params.id);
      if (!updated)
        return res.status(404).json({ message: "Return not found" });
      res.status(200).json(updated);
    } catch (error) {
      res
        .status(500)
        .json({ message: "Error completing return", error: error.message });
    }
  }
}
